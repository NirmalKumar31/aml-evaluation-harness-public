...
def test_recorded_input_hashes_are_recomputed_not_just_stored():
    ...
    # THE FLOOR SCALES WITH WHAT SHIPS. A fixed `>= 8` failed in the public
    # snapshot, where the row-level replay bundles are withheld and several
    # recorded inputs legitimately do not exist. The gate must still prove it
    # exercised everything that IS present, without inventing a shortfall.
...
def test_the_sbom_is_current_and_something_checks_it():
    ...
    if head.returncode != 0:
        return
    # ONLY IF THIS HISTORY CONTAINS THE COMMIT. The public release is a clean
    # snapshot with a single commit, so an SBOM produced in the development
    # repository names a commit this history has never seen. That is not
    # staleness -- `test_no_derived_artifact_is_stale_against_its_generator`
    # already checks the generator against HEAD -- so the ancestry check
    # applies where ancestry is a meaningful question and is skipped, without
    # a skip, where it is not.
    known = subprocess.run(["git", "-C", str(root), "cat-file", "-e", sha],
                           capture_output=True)
