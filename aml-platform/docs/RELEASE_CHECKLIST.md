...
cd aml-platform
make lint                                  # ruff
make test                                  # 422 tests collected (pass/skip
...
| 7 | Repository public | — | — | ☑ public at github.com/NirmalKumar31/aml-evaluation-harness-public, loads signed out |
...
| 9 | `main` protected | `ci`, `gates`, `image` required; force-push and deletion blocked | the ruleset id. `image` gained a `pull_request` trigger, so it can now be required | ☑ `test`, `static`, `build` required · force-push and deletion blocked · admin enforcement on |
...
| 11 | Release tagged | a signed tag and a GitHub Release exist | tag name and digest. `CITATION.cff` gets its `version`/`date-released` **only then** — a test enforces that it has neither while no tag exists | ☑ v0.1.2 tag + GitHub Release at `1f891f4`, `test`/`static`/`build` green on that SHA, and `main` fast-forwarded to it so the tag is the branch head. The tag must be PUSHED before CI runs — see HANDOFF |
...
| 15 | **Authorship identity decided** | every commit's author email is one you intend to publish | decided on the evidence rather than on preference. Measured against the PUBLIC repository's history: the institutional address appears in 1 commit author, the machine-local hostname in 0. So the hostname line published a machine name while mapping an address no public commit contains, and is removed; the institutional line stays, because deleting it would not delete an address already in the published objects. History is NOT rewritten — a rewrite cannot un-publish what is published, and the last force-push here orphaned the base and closed six Dependabot PRs | ☑ |
