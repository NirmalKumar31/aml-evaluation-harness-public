...
| 11 | Release tagged | a signed tag and a GitHub Release exist | tag name and digest. `CITATION.cff` gets its `version`/`date-released` **only then** — a test enforces that it has neither while no tag exists | ☑ v0.1.0 signed tag + GitHub Release at fa50490, all three checks green on that SHA |
